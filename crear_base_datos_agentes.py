...
db_path = os.path.join(base_dir, "agentes.db")
...
try:
    ...
    for idx, row_str in enumerate(logical_rows_str):
        ...
        # Escalafon normalization (Cell 4)
        ...
        # Turno normalization (Cell 5, 6, 7 logic combined)
        ...
        # Second Pass: Combine Turno with Plan Estudio (COL_TEMPORAL2 / COL_SINIMPORTANCIA)
    ...
    print(f"Step 4: Inserting {len(insert_data)} rows into agents database...")
    cursor.executemany("""
        INSERT INTO agentes (
            centro, establecimiento, escalafon, cupof, cue, cargo_horas, horas_catedra,
            turno, plan_estudio, nombre_agente, dni, genero, legajo, 
    ...
    cursor.execute("CREATE INDEX idx_agentes_establecimiento ON agentes(establecimiento)")
    cursor.execute("CREATE INDEX idx_agentes_revista ON agentes(situacion_revista)")
    cursor.execute("CREATE INDEX idx_agentes_escalafon ON agentes(escalafon)")
    cursor.execute("CREATE INDEX idx_agentes_horas ON agentes(horas_catedra)")
