...
/**
 * ============================================================================
 * SCRIPT: Autocompletar Depuración – DIRECCIÓN DE ÁREA TÉCNICA / CAP. LABORAL (Paso 4)
 * ============================================================================
 *
 * OBJETIVO:
 *   Vincula automáticamente registros de `depuracion_centros_sectores` en estado
 *   ACTIVO y sin vincular, correspondientes a la Dirección de Área TÉCNICA / TEC. CAP. LABORAL.
 *
 * CRITERIOS ESTRICTOS:
 *   1. Registro sin vincular (establecimiento_id IS NULL) y estado ACTIVO.
 *   2. Gestión OFICIAL únicamente (EXCLUYE PRIVADA).
 *   3. EXCLUYE JINZ / J.I.N.Z. / Inicial.
 *   4. Match en modalidades SIGE públicas (direccion_area != 'PRIVADA') en
 *      Área TÉCNICA (TEC. CAP. LABORAL, TÉCNICO, AGROTECNICA, MONOTÉCNICA, FOR. PROF.).
 *   5. Candidato único por sector (evita cualquier ambigüedad).
 * ============================================================================
 */
...
echo "{$BOLD}  AUTOCOMPLETAR DEPURACIÓN – DIRECCIÓN DE ÁREA TÉCNICA (Paso 4){$RESET}" . PHP_EOL;
echo "{$BOLD}=============================================================={$RESET}" . PHP_EOL;
...
foreach ($candidatos as $dep) {
    ...
    // Buscar modalidades de ÁREA TÉCNICA públicas en SIGE
    $modalidades = DB::table('modalidades as m')
        ...
        ->where(function($q) {
            $q->where('m.direccion_area', 'TÉCNICA')
              ->orWhereIn('m.nivel_educativo', ['TEC. CAP. LABORAL', 'TÉCNICO', 'AGROTECNICA', 'MONOTÉCNICA', 'FOR. PROF. EDUC. NO FORMAL']);
        })
        ->select(
    ...
    $vinculados++;
    $obs = "[Auto-vinculado TÉCNICA OFICIAL – Sector {$dep->sector} → CUE {$matchFinal->cue} ({$matchFinal->nivel_educativo})]";
...
echo PHP_EOL;
echo "{$BOLD}=============================================================={$RESET}" . PHP_EOL;
echo "{$BOLD}  RESUMEN TÉCNICA / CAPACITACIÓN LABORAL OFICIAL{$RESET}" . PHP_EOL;
echo "{$BOLD}=============================================================={$RESET}" . PHP_EOL;
echo "  Total examinados sin vincular : {$total}" . PHP_EOL;
echo "  {$GREEN}✔ Vinculados seguros (TÉCNICA): {$vinculados}{$RESET}" . PHP_EOL;
echo "  {$YELLOW}⚠ Ambiguos                  : {$ambiguos}{$RESET}" . PHP_EOL;
echo "  🔒 J.I.N.Z. omitidos (manual): {$jinzSaltados}" . PHP_EOL;
echo "  Sin match en TÉCNICA          : {$sinMatch}" . PHP_EOL;
echo "{$BOLD}=============================================================={$RESET}" . PHP_EOL;
...
if ($dryRun) {
    ...
    echo "{$GREEN}  ✔ Proceso completado. Se vincularon {$vinculados} registros de TÉCNICA / CAP. LABORAL.{$RESET}" . PHP_EOL;
}
