    ...
    try {
      ...
      const res = await fetch('/api/auditoria-sueldos/sanear-depuracion', {
        ...
        body: JSON.stringify({
          ...
          establecimiento_id: sanearDepEstId ? parseInt(sanearDepEstId, 10) : null,
          modalidad_id: sanearDepModalidadId ? parseInt(sanearDepModalidadId, 10) : null,
      ...
      if (res.ok) {
        ...
        alert(errData.message || 'Ocurrió un error al actualizar la depuración.');
      }
    } catch (err) {
      console.error(err);
        ...
        <Modal show={Boolean(sanearDepuracionModalItem)} onClose={() => setSanearDepuracionModalItem(null)}>
          <div className="p-6">
            ...
            <div className="mb-4 p-3 bg-gradient-to-br from-orange-50/90 via-amber-50/50 to-orange-50/40 border border-orange-200/80 rounded-2xl space-y-2.5 shadow-2xs">
              ...
              {/* Indicador de vinculación reactivo con botón directo de desvincular */}
              {(() => {
                const estSeleccionado = sanearDepEstId
                  ? establecimientosList.find((e) => e.id.toString() === sanearDepEstId.toString())
                  : null;

                const nombreEst = estSeleccionado?.nombre || (sanearDepEstId && sanearDepuracionModalItem.cue_vinculado ? sanearDepuracionModalItem.nom_establecimiento_vinculado : null);
                const cueEst = estSeleccionado?.cue || (sanearDepEstId && sanearDepuracionModalItem.cue_vinculado ? sanearDepuracionModalItem.cue_vinculado : null);

                if (sanearDepEstId && (nombreEst || cueEst)) {
                  return (
                    <div className="text-[11px] bg-emerald-50 border border-emerald-300 rounded-xl px-3 py-2 text-emerald-950 flex items-center justify-between gap-2 shadow-2xs">
                      <div className="flex items-center gap-2">
                        <i className="fa-solid fa-circle-check text-emerald-600 shrink-0 text-sm"></i>
                        <div>
                          <b>Actualmente Seleccionado:</b> {nombreEst} <span className="font-mono font-bold">(CUE {cueEst})</span>
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 text-[11px] font-black text-rose-700 bg-rose-100 hover:bg-rose-200 border border-rose-300 rounded-lg transition-all flex items-center gap-1 cursor-pointer shrink-0 shadow-2xs"
                        title="Quitar vinculación y dejar desvinculado"
                      >
                        <i className="fa-solid fa-link-slash text-[10px]"></i>
                        <span>Desvincular Escuela</span>
                      </button>
                    </div>
                  );
                }
                ...
                return (
                  <div className="text-[11px] bg-slate-100 border border-slate-300 rounded-xl px-3 py-2 text-slate-700 flex items-center justify-between gap-2 shadow-2xs">
                    <div className="flex items-center gap-2">
                      <i className="fa-solid fa-link-slash text-slate-500 shrink-0 text-sm"></i>
                      <div>
                        <b>Estado de Vinculación:</b> <span className="text-amber-800 font-bold">Sector Desvinculado</span> (Sin CUE ni escuela asignada).
                      </div>
                    </div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="block text-xs font-bold text-gray-700">Buscar y Seleccionar Escuela (CUE):</label>
                  {sanearDepEstId && (
                    <button
                      type="button"
                      onClick={() => {
                ...
                >
                  ...
                  {(() => {
                    let filtered = establecimientosList.filter((e) => {
                    ...
                    if (sanearDepEstId && !filtered.some(e => e.id.toString() === sanearDepEstId.toString())) {
                      const selectedEst = establecimientosList.find(e => e.id.toString() === sanearDepEstId.toString());
                      if (selectedEst) {
                        filtered = [selectedEst, ...filtered];
                      }
                    }
