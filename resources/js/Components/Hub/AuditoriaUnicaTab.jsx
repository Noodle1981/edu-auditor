...
export const AuditoriaUnicaTab = () => {
    ...
    const debouncedDni = useDebounce(dni, 300);
    ...
    useEffect(() => {
        const fetchSuggestions = async () => {
            const cleanQuery = debouncedDni.trim();
            if (cleanQuery.length < 3) {
                setSuggestions([]);
                return;
            }

            if (agentData && agentData.dni === cleanQuery) {
                return;
            }

            setLoadingSuggestions(true);
            try {
                const res = await fetch(`/api/agentes?search=${encodeURIComponent(cleanQuery)}&limit=5&year=${activeYear}`);
                if (res.ok) {
        ...
        fetchSuggestions();
    }, [debouncedDni, activeYear]);

    const handleSearch = async (e, targetDni = null) => {
        if (e) e.preventDefault();
        const searchDni = (targetDni || dni).trim();
        if (!searchDni) return;
        ...
        try {
            const res = await fetch(`/api/agentes/${encodeURIComponent(searchDni)}?year=${activeYear}`);
            if (!res.ok) {
                if (res.status === 404) {
                    throw new Error('No se encontró ningún agente con el dato ingresado.');
                }
                throw new Error('Ocurrió un error al consultar los datos.');
            }
            const data = await res.json();
            if (!data || !data.nombre_agente) {
                throw new Error('No se encontraron registros de planta activos para el agente ingresado.');
            }
            setAgentData(data);
            setDni(data.dni);
        } catch (err) {
    ...
    const handleSelectSuggestion = (suggestion) => {
        setDni(suggestion.dni);
        setSuggestions([]);
        setShowSuggestions(false);
        handleSearch(null, suggestion.dni);
    };
                    ...
                    <form onSubmit={(e) => handleSearch(e)} className="flex gap-3">
                        <div className="relative flex-1" ref={containerRef}>
                            ...
                            <input
                                ...
                                placeholder="DNI, Nombre o Legajo del agente..."
                            ...
                            {showSuggestions && (suggestions.length > 0 || loadingSuggestions) && (
                                <div className="absolute left-0 right-0 top-full mt-2 bg-white border border-gray-150 rounded-2xl shadow-xl z-50 overflow-hidden max-h-60 overflow-y-auto">
                                    {loadingSuggestions ? (
                                        <div className="px-4 py-3 text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2">
                                            <i className="fa-solid fa-spinner fa-spin text-[#FE8204]"></i>
                                            Buscando...
                                        </div>
                                    ) : (
                                        suggestions.map((s) => (
                                            <div
                                                ...
                                                className="px-4 py-3 hover:bg-[#FE8204]/5 cursor-pointer border-b border-gray-50 last:border-0 flex justify-between items-center transition-colors duration-150 text-left"
                                            >
                                                <div className="flex flex-col gap-0.5">
                                                    <span className="text-xs font-black text-gray-800">{s.nombre_agente}</span>
                        ...
                        <div className="border border-gray-100 rounded-2xl p-6 bg-white flex justify-between items-center shadow-sm">
                            <div>
                                ...
                                <span className={`text-xl font-black mt-1.5 block ${
                                    (agentData.auditoria.horas_activas_netas ?? 0) > 50 ? 'text-red-655' : 'text-emerald-655'
                            ...
                            {agentData.auditoria.status_auditoria === 'incompatibilidad_critica' ? (
                                <div className="flex gap-4 p-5 rounded-2xl bg-red-50/50 border border-red-100 text-red-900">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs font-semibold text-gray-650 mt-1 leading-relaxed">
                                            El docente acumula una neta de neta de <strong>{agentData.jerga_activa}</strong>, lo cual supera el tope legal sin licencias registradas hoy. Requiere citación de urgencia para regularización de cargos.
                                        </p>
                                        {agentData.cargos?.some(c => c.dato_faltante) && (
                                            <p className="text-[10px] text-red-400 mt-1 leading-relaxed font-semibold">
                                                ⚠️ Nota: {agentData.cargos.filter(c => c.dato_faltante).length} plaza(s) tienen cantidad de horas sin registrar en el sistema (dato incompleto del origen) y no se contabilizaron en este total.
                                            </p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-4 p-5 rounded-2xl bg-amber-50/50 border border-amber-100 text-amber-900">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs font-semibold text-gray-650 mt-1 leading-relaxed">
                                            El docente registra una carga total de <strong>{agentData.jerga_total}</strong>, superando los límites, pero cuenta con licencias vigentes hoy que reducen su carga activa neta a <strong>{agentData.jerga_activa}</strong>. La situación se encuentra justificada administrativamente.
                                        </p>
                                        {agentData.cargos?.some(c => c.dato_faltante) && (
                                            <p className="text-[10px] text-amber-500 mt-1 leading-relaxed font-semibold">
                                                ⚠️ {agentData.cargos.filter(c => c.dato_faltante).length} plaza(s) con dato de horas incompleto no se incluyeron en el cálculo.
                                            </p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-4 p-5 rounded-2xl bg-emerald-50/40 border border-emerald-100 text-emerald-900">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs font-semibold text-gray-650 mt-1 leading-relaxed">
                                            El agente posee una carga horaria activa de <strong>{agentData.jerga_activa}</strong>, cumpliendo con los límites de compatibilidad regulados.
                                        </p>
                                        {agentData.cargos?.some(c => c.dato_faltante) && (
                                            <p className="text-[10px] text-gray-400 mt-1 leading-relaxed font-semibold">
                                                ℹ️ {agentData.cargos.filter(c => c.dato_faltante).length} plaza(s) con dato de horas sin registrar en origen — no se incluyeron en el cálculo.
                                            </p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-gray-50 text-[10px] font-black uppercase tracking-wider text-gray-500 border-b border-gray-100">
                                            ...
                                            <th className="px-5 py-4 text-center">Horas (Equiv.)</th>
                                            <th className="px-5 py-4">Turno</th>
                                            <th className="px-5 py-4">Revista</th>
                                            <th className="px-5 py-4">Cobertura / Suplente</th>
                                    ...
                                    <tbody className="divide-y divide-gray-100 text-xs text-gray-750">
                                        {agentData.cargos.map((cargo, idx) => (
                                            <tr key={cargo.id || idx} className="hover:bg-gray-50/40">
                                                ...
                                                <td className="px-5 py-4 font-black text-center text-gray-900">
                                                    {cargo.horas_equivalentes ?? cargo.horas_catedra ?? 0} Hs.
                                                    {cargo.etiqueta_equivalencia && cargo.etiqueta_equivalencia !== 'Horas Cátedra' && (
                                                        <span className="block text-[9px] text-[#FE8204] font-medium leading-none mt-1">
                                                            {cargo.etiqueta_equivalencia}
                                                        </span>
                                                    )}
                                                </td>
                                                ...
                                                <td className="px-5 py-4 leading-normal">
                                                    {cargo.estado_cobertura === 'cubierto_suplente' ? (
                                                        <div className="flex flex-col gap-0.5">
                                                            ...
                                                            <span className="text-[10px] text-gray-900 font-black">
                                                                {cargo.suplente_nombre}
                                                            </span>
                                                            <span className="text-[9px] text-gray-400 font-mono font-bold">
                                                                DNI: {cargo.suplente_dni}
                                                            </span>
                                                        </div>
                                                    ) : cargo.estado_cobertura === 'licencia_sin_suplente_db' ? (
